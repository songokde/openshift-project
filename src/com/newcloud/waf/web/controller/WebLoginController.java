					...
					if(errorCode == 1){
						...
						if(!HttpSessionCache.getLocalSession(session.getId())) {
							...
							if(user!=null) {
								...
								Map<Integer,String> domainMap = userService.findComboxMap(InfoDomainname.class, "dniId", "dniDname","dniCusid=?",new Object[] {customer.getCusId()});
								//存放SESSION
								session.setAttribute("SESSION_USER", user);
								session.setAttribute("SESSION_CUSTOMER", customer);
								session.setAttribute("SESSION_DOMAIN", domainMap);
								//存储对应关系
